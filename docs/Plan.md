...
- **M5.1 예산·수렴 제어 (D-032, 기능 구현 완료·실안건 튜닝 진행 중)**:
  ...
  - 실안건 튜닝 완료 기준: 대표 기술 안건 10개 중 8개 이상 완료, 작업 토큰 중앙값 40k 이하,
    ...
    표본은 실사용 세션으로 자연 누적한다(D-039) — 현재 1/10.
...
- **후속 견고화 (운영 관찰 기간, D-039)**: 신규 기능 개발은 보류하고 실사용
  세션으로 관찰 데이터를 쌓는다. 아래 항목은 각자의 착수 트리거가 성립할 때
  진행한다.
  - **종료 경합 재검증 (보류 — D-039)**: 취소 vs 승인, 승인 vs 예산 최종 정산의
    경합을 밀폐 재현하는 테스트를 먼저 만들고, 그 결과를 근거로 first-wins 확정
    (D-021 개정) 또는 우선순위 중재 도입을 재결정한다. M5.3의 반려 시 호출
    취소·늦은 투표 거부와 idle/voting 타이머 무경합은 이미 회귀 테스트로 커버됨.
    현재 모든 경합의 최악 결과는 안전(초안 보존, 상태 무결)하다.
  - **이력 compaction — 요약 기반 (관측 후 결정 — D-038)**: 절단 발생은 경고
    로그로 관측 중(agent.py, 상한 60턴). voting 문맥은 D-035/D-037로 결정론적
    압축이 완료됐고, 남은 범위는 일반 토론 이력의 요약 압축과 §2/D-025 보존
    우선순위(제안·투표 우대, 최신 제안 1개 원문 보호)다. 착수 트리거: 실안건에서
    절단 로그가 반복 관측되거나, M6 작업 도구 도입으로 이력 팽창이 예정될 때 —
    M6와 겹치면 그 시점의 재설계와 통합한다.
  - **SQLite 이벤트 backlog 페이지 단위 전송·compaction (M3 이월)**: 현재 SSE
    재접속 시 store의 과거 이벤트를 전량 재전송한다. 세션당 이벤트가 수백 건
    수준이라 시급도 낮음. 착수 트리거: 세션당 이벤트가 수천 건 규모로 커지거나
    (M6 도구 작업), 재접속 지연이 체감될 때.
  - **서버 재시작 고도화 — 실행 중 세션 복원**: 현재는 failed(interrupted) +
    마지막 제안 초안 보존(D-025)으로 종결한다. 실측 세션 길이(~2분)에서는
    재시작 손실이 작아 비착수(D-039). 착수 트리거: M6 장시간 도구 작업으로
    세션이 길어질 때.
  - **E2E (실 스택)**: 자연 누적되는 실안건 튜닝 세션이 실 스택 검증을 겸한다
    (D-039). 별도 후보는 반복 실행 가능한 얇은 스모크 스크립트(서버 기동 → 제출
    → SSE 구독 → 결과 확인) 1개 — 회귀 안전망이 필요해지면 추가한다.
  - **문서-코드 정합**: 2026-07-15 계획-구현 전수 검증 완료(검증 가능 주장 42건
    중 39 일치, 표류 3건 정정 — WorkLog 참조). 이후는 마일스톤 완료 시 3렌즈
    검토로 유지한다.
