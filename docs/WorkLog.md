...
### 남은 결정(사용자) — 전부 처리됨 (2026-07-15)
- ~~§2 절단 보존 우선순위의 구현 시기~~ — **D-038로 해소**: 절단 관측 로그만
  추가하고 실안건 튜닝 기간 실측 빈도로 재결정 (agent.py에 경고 로그 추가).
- ~~M5.1 실안건 튜닝 / 후속 견고화 순서~~ — **D-039로 해소**: 튜닝 표본은
  실사용 자연 누적(현재 1/10), 당분간 운영 관찰 기간으로 큰 개발 보류.
- 종료 경합 우선순위표 — **보류** (D-039 §3): 경합 재현 테스트 후 first-wins
  확정 여부 재결정. Plan §6에 미결로 표기 유지.
